...
class Commands(commands.Cog):
    ...
    async def changePrefix(self, ctx, prefix):
        ...
        if role in ctx.author.roles:
            if len(prefix) == 1:
                ...
                await ctx.send(f"Hey you can't put a prefix that's more than one character!")
        else:
        ...
        embed = discord.Embed(
            description=f"The current prefix is: {prefixes[str(ctx.guild.id)]}",
            colour=discord.Colour.purple()
        )
        # embed.set_author(name="Scarlett",
        #                  icon_url='https://cdn.discordapp.com/attachments/640127172148985861/661150910055186472/spider.png')
    ...
    #     await ctx.send('You passed {} arguments: {}'.format(len(args), ', '.join(args)))
